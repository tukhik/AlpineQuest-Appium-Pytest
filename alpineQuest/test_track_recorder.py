...
def test_track_recorder(application):
    ...
    track_recorder = gps_page.find_element_by_xpath((AppiumBy.XPATH, GPS_PAGE.SWITCH_TRACK_RECORDER_OFF))

    track_recorder.click()

    # Step 6:  Find LOCK icon
    gps_page.wait_for_visible_element(GPS_PAGE.LOCK_ICON)
    lock_icon = gps_page.find_element_by_xpath((AppiumBy.XPATH, GPS_PAGE.LOCK_ICON))
    assert lock_icon.is_displayed(), "lock icon is not visible or not clickable"

    # Step 7:  Find CANCEL button
    cancel_button = gps_page.find_element_by_xpath((AppiumBy.XPATH, GPS_PAGE.CANCEL_BUTTON))
