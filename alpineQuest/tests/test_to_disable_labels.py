...
def test_to_disable_labels(application):
    # Step 1: Open the app.
    ...
    # Step 11: Click eye icon
    eye_icon = place_mark_page.find_by_xpath(PLACE_MARKS_PAGE.EYE_ICON_SELECTOR)
    eye_icon.click()

    # Step 11: Delete waypoint
    place_mark_page.long_press_button(created_waypoint)
    delete_button = place_mark_page.find_by_xpath(PLACE_MARKS_PAGE.DELETE_SELECTOR)
    assert delete_button.is_displayed(),  "delete_button is not found"
    delete_button.click()

    yes_button = place_mark_page.find_by_xpath(PLACE_MARKS_PAGE.YES_BUTTON_SELECTOR)
    assert yes_button.is_displayed(), "yes_button is not found"
    yes_button.click()

    # Step 12: Click x button and close modal
